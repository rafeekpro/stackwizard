...
export const ErrorMessages = {
  ...
  [ErrorCodes.PERMISSION_DENIED]: {
    ...
    solution: 'Run the command with appropriate permissions or choose a different directory',
    recoverable: true
  },
  [ErrorCodes.GIT_NOT_INSTALLED]: {
    message: 'Git is not installed',
    solution: 'Install Git from https://git-scm.com/ or skip Git initialization',
    recoverable: true
  },
  ...
  [ErrorCodes.PIP_INSTALL_FAILED]: {
    ...
    solution: 'Try running "pip install -r requirements.txt" manually in the backend directory',
    recoverable: true
  },
  ...
  [ErrorCodes.UNKNOWN_ERROR]: {
    ...
    solution: 'Please report this issue at https://github.com/rafeekpro/stackwizard/issues',
    recoverable: false
  },
...
export class ErrorHandler {
  ...
  determineErrorCode(error) {
    ...
    if (message.includes('eacces') || message.includes('permission')) return ErrorCodes.PERMISSION_DENIED;
    ...
    if (message.includes('network') || message.includes('enotfound')) return ErrorCodes.NETWORK_ERROR;
  ...
  async attemptRecovery(errorCode, error, context) {
    ...
    this.logger.info(`Attempting recovery (attempt ${this.recoveryAttempts[recoveryKey]}/3)`);
   ...
   */
  async recoverFromDirExists(context) {
  ...
  async recoverFromNetworkError(context) {
    ...
    console.log(chalk.gray('  • Backend: cd backend && pip install -r requirements.txt'));
...
export const Validators = {
  projectName: (input) => {
    ...
    const reserved = ['node_modules', 'dist', 'build', 'test', 'tests', '.git', '.env', 'src', 'lib', 'bin'];
...
export async function checkSystemRequirements(logger) {
  const requirements = {
    node: { command: 'node --version', name: 'Node.js', optional: false, minVersion: '14.0.0' },
    npm: { command: 'npm --version', name: 'npm', optional: false, minVersion: '6.0.0' },
    git: { command: 'git --version', name: 'Git', optional: true },
    docker: { command: 'docker --version', name: 'Docker', optional: true },
    python: { command: 'python3 --version || python --version', name: 'Python', optional: true, minVersion: '3.8.0' },
    pip: { command: 'pip3 --version || pip --version', name: 'pip', optional: true }
  };
  ...
  for (const [key, req] of Object.entries(requirements)) {
    try {
      ...
      if (!req.optional) {
        throw new StackWizardError(
          ErrorCodes[`${key.toUpperCase()}_NOT_INSTALLED`] || ErrorCodes.COMMAND_NOT_FOUND,
...
export async function cleanup(projectPath, logger) {
  if (projectPath && fs.existsSync(projectPath)) {
    try {
      ...
      if (stats.size > 1024 * 1024) { // If > 1MB, create backup
      ...
      console.log(chalk.red('⚠️  Failed to clean up. Please manually remove: ' + projectPath));
    }
  }
}
...
function extractVersion(output) {
  const versionMatch = output.match(/(\d+\.\d+\.\d+)/); return versionMatch ? versionMatch[1] : null;
}
